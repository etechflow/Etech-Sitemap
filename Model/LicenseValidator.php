...
/**
 * Portal license validator for Etechflow_Sitemap.
...
{
    public const XML_PATH_LICENSE_KEY            = 'etechflow_sitemap/license/license_key';
    public const XML_PATH_PRODUCTION_ENVIRONMENT = 'etechflow_sitemap/license/production_environment';
    public const XML_PATH_PORTAL_URL             = 'etechflow_sitemap/license/portal_url';
    public const XML_PATH_PORTAL_API_URL         = 'etechflow_sitemap/license/portal_api_url';

    public const XML_PATH_BUNDLE_LICENSE_KEY = 'etechflow_bundle/license/license_key';

    /**
     * Retained config paths written by the activation controller. isValid()
     * never consults them (the client-settable "locally issued" grace was
     * removed as a forgery surface); they are kept only so existing writers
     * keep resolving.
     */
    public const XML_PATH_ISSUED_KEY = 'etechflow_sitemap/license/issued_key';
    public const XML_PATH_ISSUED_AT  = 'etechflow_sitemap/license/issued_at';
    public const XML_PATH_IP_BLOCKED = 'etechflow_sitemap/license/ip_blocked';

    private const MODULE_ID = 'sitemap';
    ...
    private const CACHE_TAG = 'etechflow_sitemap_license';
    ...
    {
        $cacheKey = 'etf_sitemap_lic_' . md5($host . ':' . $licenseKey);
        ...
        try {
            ...
            $this->curl->addHeader('User-Agent', 'ETechFlow-Sitemap/1.0');
            $this->curl->get($url);
    ...
    {
        return 'etf_sitemap_lic_grace_' . md5($this->canonicalize($host) . ':' . $licenseKey);
    }
    ...
    private function isExplicitlyRevoked(): bool
    {
        return (string) $this->scopeConfig->getValue(
            'etechflow_sitemap/license/revoked',
